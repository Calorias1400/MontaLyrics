...
def exportar_xml(tl, imgs_dir, output, fps):
    ...
    # CORRECCIÓN: Calcular el total de pistas necesarias sumando todas
    total_pistas = sum(len(bloque) for bloque in bloques)
    ...
    # CORRECCIÓN: Crear pistas y asignar clips correctamente
    # Primero, crear una lista de todos los clips con su pista global
    clips_con_pistas = []
    pista_global = 1
    
    for bloque_idx, bloque in enumerate(bloques):
        pista_en_bloque = 1  # Cada bloque empieza en V1
    ...
    # Crear las pistas físicas basadas en el máximo de clips por bloque
...
class App:
    def __init__(self, root):
        root.title("🎯 PISTAS CORREGIDAS - Reinicia V1 por bloque")
        ...
        Button(root, text="🎯 GENERAR - REINICIA V1", command=self.run, bg='#28a745', fg='white').grid(row=5, column=0, columnspan=2, sticky="ew")
    ...
    def run(self):
        try:
            ...
            out     = self.paths.get('dest') or 'REINICIA_V1_POR_BLOQUE.xml'
            exportar_xml(tl, self.paths['imgs'], out, fps)
            messagebox.showinfo('✅ PISTAS CORREGIDAS!',f'Cada bloque reinicia desde V1!\n\nArchivo: {out}')
