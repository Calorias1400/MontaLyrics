...
def exportar_xml(tl, imgs_dir, output, fps):
    ...
    # Construir XML línea por línea para evitar problemas con las etiquetas
    xml_lines = []
    xml_lines.append('<?xml version="1.0" encoding="UTF-8"?>')
    xml_lines.append('<!DOCTYPE xmeml>')
    xml_lines.append('<xmeml version="1">')
    xml_lines.append('<project>')
    xml_lines.append('<name>Auto Premiere Sequence</name>')
    xml_lines.append('<children>')
    xml_lines.append('<sequence id="autoseq">')
    xml_lines.append('<name>Auto Sequence</name>')
    xml_lines.append(f'<duration>{total_duration}</duration>')
    xml_lines.append('<rate>')
    xml_lines.append(f'<timebase>{fps}</timebase>')
    ...
    xml_lines.append('<source>source</source>')
    ...
    for track_num in range(1, max_pistas + 1):
        ...
        xml_lines.append('<enabled>TRUE</enabled>')
        xml_lines.append('<locked>FALSE</locked>')
        
        # Agregar clips a la pista actual
        for bloque_idx, bloque in enumerate(bloques):
            if track_num <= len(bloque):
                clip = bloque[track_num - 1]  # track_num es 1-indexed
                
                # Calcular el índice correcto de la imagen
                png_index = sum(len(b) for b in bloques[:bloque_idx]) + (track_num - 1)
                ...
                if png_index < len(pngs):
                    ...
                    xml_lines.append(f'<clipitem id="clipitem-{track_num}-{bloque_idx}">')
                    xml_lines.append(f'<name>{src_file.name}</name>')
                    xml_lines.append('<enabled>TRUE</enabled>')
                    xml_lines.append(f'<duration>{duration_frames}</duration>')
                    xml_lines.append(f'<start>{start_frame}</start>')
                    xml_lines.append(f'<end>{end_frame}</end>')
                    xml_lines.append('<in>0</in>')
                    xml_lines.append(f'<out>{duration_frames}</out>')
                    xml_lines.append(f'<file id="file-{png_index}">')
                    xml_lines.append(f'<name>{src_file.name}</name>')
                    xml_lines.append(f'<pathurl>file://localhost{src_file.resolve()}</pathurl>')
                    xml_lines.append('<rate>')
                    xml_lines.append(f'<timebase>{fps}</timebase>')
                    xml_lines.append('<ntsc>FALSE</ntsc>')
                    xml_lines.append('</rate>')
                    ...
                    xml_lines.append('<trackindex>1</trackindex>')
                    xml_lines.append('</sourcetrack>')
...
class App:
    def __init__(self, root):
        root.title("Auto Premiere Sequence XML")
