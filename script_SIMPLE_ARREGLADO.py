...
def exportar_xml(tl, imgs_dir, output, fps, bloques_originales):
    ...
    # Usar los bloques originales pasados como parámetro
    clip_counts_por_bloque = [len(subs_bloque) for ini, fin, subs_bloque in bloques_originales]
    
    # CLAVE: Solo crear pistas según el bloque más grande
    ...
    xml_lines.extend([
        ...
        '<xmeml version="1">',
        '<project>',
        '<name>Secuencia SIMPLE ARREGLADA</name>',
        '<children>',
...
class App:
    def __init__(self, root):
        root.title("✅ SIMPLE ARREGLADO - Solo pistas necesarias")
        ...
        Button(root, text="✅ SIMPLE ARREGLO", command=self.run, bg='#28a745', fg='white').grid(row=5, column=0, columnspan=2, sticky="ew")
    ...
    def run(self):
        try:
            ...
            out     = self.paths.get('dest') or 'SIMPLE_ARREGLADO.xml'
            # CLAVE: Pasar los bloques originales a la función
            exportar_xml(tl, self.paths['imgs'], out, fps, bloques)
            messagebox.showinfo('✅ SIMPLE ARREGLADO!','Solo las pistas necesarias creadas!\n\nArchivo: {}'.format(out))
        except Exception as e:
            messagebox.showerror('❌ Error', str(e))

if __name__ == '__main__':
    root = Tk(); root.geometry('470x300')
    App(root)
    root.mainloop()