...
def exportar_xml(tl, imgs_dir, output, fps):
    ...
    # CORRECCIÓN: XML con configuración exacta de secuencia
    xml_content = []
    ...
    # CONFIGURACIÓN EXACTA DE SECUENCIA VERTICAL
    ...
    for track_num in range(1, max_pistas + 1):
        ...
        for bloque_idx, bloque in enumerate(bloques):
            if track_num <= len(bloque):
                clip = bloque[track_num - 1]
                
                # Calcular índice correcto de imagen
                clip_index_in_block = track_num - 1
                ...
                if global_image_index < len(pngs):
                    ...
                    xml_content.append('<ntsc>FALSE</ntsc>')
                    xml_content.append('</rate>')
                    # CADA CLIP TAMBIÉN VERTICAL 1080x1920
                    xml_content.append('<width>1080</width>')
                    xml_content.append('<height>1920</height>')
                    xml_content.append('<anamorphic>FALSE</anamorphic>')
                    xml_content.append('<pixelaspectratio>square</pixelaspectratio>')
    ...
    # AUDIO BÁSICO PARA COMPLETAR LA SECUENCIA
...
class App:
    def __init__(self, root):
        root.title("🎬 Premiere XML Vertical (1080x1920) - DEFINITIVO")
        ...
        Button(root, text="🎥 GENERAR VERTICAL XML", command=self.run, bg='#ff6b35', fg='white').grid(row=5, column=0, columnspan=2, sticky="ew")
    ...
    def run(self):
        try:
            ...
            out     = self.paths.get('dest') or 'VERTICAL_1080x1920.xml'
            exportar_xml(tl, self.paths['imgs'], out, fps)
            messagebox.showinfo('✅ VERTICAL GENERADO!',f'Secuencia 1080x1920 a {fps}fps creada!\n\nImporta: {out}')
        except Exception as e:
            messagebox.showerror('❌ Error', str(e))

if __name__ == '__main__':
    root = Tk(); root.geometry('380x300')
    App(root)
    root.mainloop()